...
def chang_date(days_add=1200):
    """ Max date a stock must have
        begin existing"""
    global min_date, current_date, end_date
    min_date = current_date + datetime.timedelta(days=days_add)
...
def buy(stock_name, date, code, when_sell):
    ...
    if date >= current_date and total > 0:
        ...
        transactions.append(str(date) + ' buy-low ' + stock_name.split(sep='.')[0].upper() + " " + str(total))
        ...
        if current_date > convert_date('1975-01-01'):
...
def sell(date, stock_name, code):
    ...
    transactions.append(str(date) + ' sell-high ' + stock_name.split(sep='.')[0].upper() + " " + str(total))
    total_money += total * frame.at[date, code]
